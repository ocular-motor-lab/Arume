classdef JDPursuit < ArumeExperimentDesigns.EyeTracking
    ...
    methods ( Access = protected )
        ...
        function dlg = GetOptionsDialog( this, importing )
            ...
            %% ADD new options
            dlg.Correct_Speed_TEST_deg = { 4.8 '* (deg/s)' [-100 300]};
                ...
                while secondsRemaining > 0
                   ...
                   secondsRemaining    = this.ExperimentOptions.TrialDuration - secondsElapsed;
                    ...
                    elseif (secondsElapsed >= this.stimOffFixate_seconds && secondsElapsed < (this.stimOffFixate_seconds+this.stimOffPursue_seconds)) %moving but no stimuli
                        ...
                        if thisTrialData.NOPursueExp == 1
                    ...
                    elseif ( secondsElapsed >= (this.stimOffFixate_seconds+this.stimOffPursue_seconds) && secondsElapsed <= (this.stimOffFixate_seconds+this.stimOffPursue_seconds + this.stimOnPursue_seconds))
                        ...
                        if thisTrialData.StartingFromLeft == 0
                            if thisTrialData.GrowingOutwards == 1 %normal
                                ...
                                velocity_pix = abs(thisTrialData.Speed_ref_pix-thisTrialData.Speed_middle_pix);
                                [driftGrating, imSize] = makeDG(graph.window, velocity_pix, (secondsElapsed-reset_secondsElapsed), 255);
                                destinationRectangle = CenterRectOnPointd([0 0 imSize imSize], (secondsElapsed-reset_secondsElapsed)*  thisTrialData.Speed_middle_pix+reset_secondsElapsed_middle +this.allStimShift-thisTrialData.LRoffset, my);
                                Screen('DrawTexture', graph.window, driftGrating, [],destinationRectangle);
                            ...
                            if thisTrialData.GrowingOutwards == 1 %normal
                                ...
                                velocity_pix = abs(thisTrialData.Speed_TEST_pix-thisTrialData.Speed_middle_pix);
                                [driftGrating, imSize] = makeDG(graph.window, velocity_pix, (secondsElapsed-reset_secondsElapsed), 255);
                                destinationRectangle = CenterRectOnPointd([0 0 imSize imSize], (secondsElapsed-reset_secondsElapsed)*  thisTrialData.Speed_middle_pix+reset_secondsElapsed_middle +this.allStimShift-thisTrialData.LRoffset, my);
                                Screen('DrawTexture', graph.window, driftGrating, [],destinationRectangle);
                        ...
                        % % % Screen('FillRect', graph.window,  this.stimColor, fixRect);
                                ...
                                if thisTrialData.control_Fix_noMove == 1
                                    if thisTrialData.randEye == 0
                                        % [Adding left--fixation circle in center]----------------------------------------------
                                        fixCirc_size = [0 0 this.fix_size this.fix_size];
                                        fixRect = CenterRectOnPointd( fixCirc_size,  mx, my);%+  (this.blackRing_half_length-this.insideCircle_half_length)/2+this.insideCircle_half_length );
        ...
        %%%
                    ...
                    elseif (secondsElapsed > (this.stimOffFixate_seconds+this.stimOffPursue_seconds + this.stimOnPursue_seconds) && secondsElapsed <= (this.stimOffFixate_seconds+this.stimOffPursue_seconds + this.stimOnPursue_seconds + this.stimFadeOut_seconds))
                        ...
                        FadeToWhiteColor = [fadeC fadeC fadeC].*255/2;
                    ...
                    if ( secondsElapsed > (this.stimOffFixate_seconds+ this.stimOffPursue_seconds + this.stimOnPursue_seconds ) ) %not including fade out so that you can respond during fade out  + this.stimFadeOut_seconds
            ...
            function [reset_secondsElapsed] = fadeOut(this, thisTrialData, secondsElapsed, reset_secondsElapsed, fade_reset_secondsElapsed, graph)
                t =  1-min((secondsElapsed-fade_reset_secondsElapsed) / this.stimFadeOut_seconds, 1);
                FadeToWhiteColor = t *255; %[255/2 255/2 255/2]; %
                ...
                if thisTrialData.StartingFromLeft == 0
                    if thisTrialData.GrowingOutwards == 1 %normal
                        % fixRect = CenterRectOnPointd( this.stimRect_size, (secondsElapsed-reset_secondsElapsed)*  thisTrialData.Speed_ref_pix-thisTrialData.LRoffset+reset_secondsElapsed_middle +this.allStimShift, my );
                        velocity_pix = abs(thisTrialData.Speed_ref_pix-thisTrialData.Speed_middle_pix);
                        [driftGrating, imSize] = makeDG(graph.window, velocity_pix, (secondsElapsed-reset_secondsElapsed), FadeToWhiteColor);
                        destinationRectangle = CenterRectOnPointd([0 0 imSize imSize], (secondsElapsed-reset_secondsElapsed)*  thisTrialData.Speed_middle_pix+reset_secondsElapsed_middle +this.allStimShift-thisTrialData.LRoffset, my);
                    else %white bar shrinks
                        % fixRect = CenterRectOnPointd( this.stimRect_size, (secondsElapsed-reset_secondsElapsed)*  thisTrialData.Speed_middle_pix-thisTrialData.LRoffset+reset_secondsElapsed_middle +this.allStimShift      - predicted_lead* (1-(secondsElapsed-reset_secondsElapsed)/this.stimOnPursue_seconds), my );
                ...
                % jdColor = [255 0 0];
                % Screen('FillRect', graph.window,  FadeToWhiteColor, fixRect);
                % driftGrating = driftGrating./255 * FadeToWhiteColor;
                ...
                for frame = 1: size(noisejpgread_crop,1)
                    for c = 1: size(noisejpgread_crop,2)
                        if noisejpgread_crop(frame,c,1) == 255
                            noisejpgread_crop(frame,c,1) = lumJDset(1)/2; %this number is for 4.2e-01 luminance
                            noisejpgread_crop(frame,c,2) = lumJDset(2)/2; %this number is for 4.2e-01 luminance
                            noisejpgread_crop(frame,c,3) = lumJDset(3)/2;%this number is for 4.2e-01 luminance
                        end
                    end
                end
            ...
            function [mx] = setCowBkgdwithCross(graph, mx, my, this, tex1oF, insideColor)
                ...
                if this.blankwhite == 1
                    ...
                    if this.turnOnRing == 1
                        blackRing_rgb = [0 0 0]; %[1 1 1]; %
                        % Drawing the black circle first so that it becomes ring
                        Screen('FillRect',graph.window,grey,... %black oval
                            [1,...
            ...
            function [driftGrating, imSize] = makeDG(w, velocity_pix, secElapsed, colorVal)


                % Parameters
                imSize = 120;          % Image size (pixels)
                sf = 0.05;             % Spatial frequency (cycles per pixel)
                theta = 0;            % Orientation (degrees)
                sigma = (40/256)* imSize; %40;            % Width of Gaussian envelope (standard deviation in pixels)
                phaseDelta = 0.2;      % Speed of drift (phase shift per frame)
                ...
                % Pre-calculate the ramp for the grating orientation
                % This defines the "direction" the waves travel
                ramp = (x - x0) * cos(angleRad) + (y - y0) * sin(angleRad);

                % Calculate current phase
                % currentPhase = frame * phaseDelta;
                currentPhase = 2 * pi * sf * velocity_pix * secElapsed;


                % Generate grating: cos(2 * pi * sf * ramp + phase)
                grating = cos(2 * pi * sf * ramp + currentPhase);

                % Multiply by Gaussian to create the Gabor
                gabor = grating .* gaussian;

                gabor = (gabor + 1)./2*colorVal; %jd add

                driftGrating = Screen('MakeTexture', w, gabor);
