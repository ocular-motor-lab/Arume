classdef JDPursuit < ArumeExperimentDesigns.EyeTracking
    ...
    properties
        ...
        stimRect_size =  [0 0 45 130]; % [0 0 45 112]; %pixels
        fix_size = 20; %pixels
        LRoffset = 100; %pixels
    ...
    methods ( Access = protected )
        ...
        function dlg = GetOptionsDialog( this, importing )
            ...
            %% ADD new options
            dlg.Correct_Speed_TEST_deg = { 4.8 '* (deg/s)' [1 300]};
        ...
        function trialTable = SetUpTrialTable(this)
            ...
            this.ExperimentOptions.SpeedStep_deg = this.ExperimentOptions.SpeedStep_pix/ppd; %updates if any of the while loops were used
            ...
            try
                ...
                while secondsRemaining > 0
                   ...
                   secondsRemaining    = this.ExperimentOptions.TrialDuration - secondsElapsed;
                    ...
                    if ( secondsElapsed < this.stimOffFixate_seconds)
                        ...
                        if this.control_Fix_noMove == 0
                            ...
                            if thisTrialData.StartingFromLeft == 0 %Right
                                this.allStimShift = mx+round(predicted_travel_distance_pix/2)+pursueDist;
                            else
                                this.allStimShift = mx-round(predicted_travel_distance_pix/2)-pursueDist;
                            end
                    ...
                    elseif (secondsElapsed >= this.stimOffFixate_seconds && secondsElapsed < (this.stimOffFixate_seconds+this.stimOffPursue_seconds))
                        ...
                        if this.control_Fix_Left ==1
                            %LEFT
                            if thisTrialData.StartingFromLeft == 0
                                fixRect = CenterRectOnPointd( this.stimRect_size, (secondsElapsed - first_reset_secondsElapsed)*thisTrialData.Speed_Ref_pix-this.LRoffset +this.allStimShift, my );
                            else
                                fixRect = CenterRectOnPointd( this.stimRect_size, (secondsElapsed - first_reset_secondsElapsed)*thisTrialData.Speed_TEST_pix-this.LRoffset +this.allStimShift, my ); %start left
                            end
                            ...
                            Screen('FillRect', graph.window,  this.stimColor, fixRect);
                                    ...
                                    % [Adding left--fixation circle in center]----------------------------------------------
                                    fixCirc_size = [0 0 this.fix_size this.fix_size];
                                    fixRect = CenterRectOnPointd( fixCirc_size,   (secondsElapsed - first_reset_secondsElapsed)*thisTrialData.Speed_TEST_pix-this.LRoffset +this.allStimShift, my );
                                    Screen('FillOval', graph.window,  [255 255 255], fixRect);
        
                                    fixCross_size = [0 0 this.fix_size/4 this.fix_size];
                                    fixRect = CenterRectOnPointd( fixCross_size,   (secondsElapsed - first_reset_secondsElapsed)*thisTrialData.Speed_TEST_pix-this.LRoffset +this.allStimShift, my );
                                    Screen('FillOval', graph.window,  [0 0 0], fixRect);
                        ...
                        elseif this.control_Fix_noMove == 1
                            ...
                            if thisTrialData.StartingFromLeft == 0 
                                this.allStimShift = mx+round(predicted_travel_distance_pix/2)+pursueDist;
                                if this.endAtCross==1
                                    this.allStimShift = mx+round(predicted_travel_distance_pix)+pursueDist -this.fix_size/2;%jd1/29
                                end
                            else
                                this.allStimShift = mx-round(predicted_travel_distance_pix/2)-pursueDist;
                                 if this.endAtCross==1
    ...
    methods ( Access = public )
        ...
        function Plot_Psychometric(this)
            ...
            saveas(gcf, [saveFolder, '/', this.Session.subjectCode, '_', this.Session.sessionCode '_PF.jpg'])
